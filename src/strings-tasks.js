...
function isPalindrome(str) {
  ...
  if (str.split(' ').length > 1) {
    const newStr = str
      ...
      .join('');
    return newStr === newStr.split('').toReversed().join('');
  }
  const strReverse = str.toLowerCase().split('').toReversed().join('');
  return str.toLowerCase() === strReverse;
}
...
function encodeToRot13(str) {
  ...
  return str
    .split('')
    .map((letter) => {
      if (!arrSymbol.includes(letter)) {
        if (letter === letter.toUpperCase()) {
          let positionCharCode = letter.charCodeAt() + 13;
          if (positionCharCode > 90) {
            positionCharCode = positionCharCode - 90 - 1 + 65;
            return String.fromCodePoint(positionCharCode);
          }
          return String.fromCodePoint(positionCharCode);
        }
        let positionCharCodeSmall = letter.charCodeAt() + 13;
        if (positionCharCodeSmall > 122) {
          positionCharCodeSmall = positionCharCodeSmall - 122 - 1 + 97;
          return String.fromCodePoint(positionCharCodeSmall);
        }
        return String.fromCodePoint(positionCharCodeSmall);
      }
      return letter;
